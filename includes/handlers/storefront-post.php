...
function storefrontHandleBlogPostPost(mysqli $conn): void
{
    ...
    if (!in_array($action, ['submit_comment', 'admin_comment_reply'], true)) {
        return;
    }
    ...
    } elseif ($action === 'admin_comment_reply') {
        if (!adminCurrent()) {
            $notice = 'Bạn cần đăng nhập quản trị để trả lời.';
        } else {
            $result = blogCommentAdminReply($conn, (int) ($_POST['parent_id'] ?? 0), (string) ($_POST['reply_content'] ?? ''));
...
function storefrontHandleProductPost(mysqli $conn): void
{
    ...
    if (!csrfValidate()) {
        ...
        $notice = $result['message'];
        $success = $result['ok'];
    } elseif ($action === 'admin_review_reply') {
        if (!adminCurrent()) {
            $notice = 'Bạn cần đăng nhập quản trị để trả lời.';
        } else {
            $result = reviewAdminReply($conn, (int) ($_POST['parent_id'] ?? 0), (string) ($_POST['reply_content'] ?? ''));
            $notice = $result['message'];
            $success = $result['ok'];
        }
    }
    ...
    $hash = in_array($action, ['submit_review', 'admin_review_reply'], true) ? '#product-reviews' : '';
    redirect(app_url('product', ['slug' => $slug]) . $hash);
}
