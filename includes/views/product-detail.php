...
$reviewNotice = $productFlash['message'];
...
<section class="container product-page" data-product-detail>
    ...
    <?php if ($productAdminView): ?>
        <div class="product-admin-bar">
            <p>Bạn đang xem với tài khoản quản trị. Cuộn xuống tab <strong>Đánh giá</strong> để trả lời khách ngay dưới từng đánh giá.</p>
        ...
        <div class="product-tabs-panels">
            ...
            <article class="product-tab-panel" id="product-reviews" data-panel="reviews" role="tabpanel" hidden>
                ...
                <?php if (empty($reviews)): ?>
                    <p class="reviews-empty">Chưa có đánh giá. Hãy là người đầu tiên!</p>
                <?php else: ?>
                    <ul class="review-list">
                        <?php foreach ($reviews as $review): ?>
                            <li class="review-item">
                                ...
                                <?php
                                $threadReplies = $review['replies'] ?? [];
                                include __DIR__ . '/partials/review-thread.php';
                                ?>
                                <?php if ($productAdminView): ?>
                                    <form method="post" action="<?php echo e(app_url('product', ['slug' => $product['slug']])); ?>#product-reviews" class="storefront-admin-reply-form">
                                        <?php echo csrfField(); ?>
                                        <input type="hidden" name="action" value="admin_review_reply">
                                        <input type="hidden" name="parent_id" value="<?php echo (int) $review['id']; ?>">
                                        <label for="review-reply-<?php echo (int) $review['id']; ?>">Trả lời khách (Winsum Home)</label>
                                        <textarea id="review-reply-<?php echo (int) $review['id']; ?>" name="reply_content" rows="3" maxlength="2000" required placeholder="Nhập phản hồi của shop..."></textarea>
    ...
    <?php if (!empty($related)): ?>
        <section class="related-products" aria-labelledby="related-products-heading">
            <h2 id="related-products-heading">Sản phẩm liên quan</h2>
            <div class="related-products-grid">
                <?php foreach ($related as $item): ?>
                    <article class="related-product-card">
                        <a href="<?php echo e(app_url('product', ['slug' => $item['slug']])); ?>" class="related-product-card__image">
                        ...
                        <div class="related-product-card__body">
                            <h3 class="related-product-card__title">
                                <a href="<?php echo e(app_url('product', ['slug' => $item['slug']])); ?>"><?php echo htmlspecialchars($item['name']); ?></a>
                            </h3>
                            <p class="related-product-card__price"><?php echo htmlspecialchars($item['price_label']); ?></p>
                        </div>
                    </article>
                <?php endforeach; ?>
            </div>
